...
const start = async () => {
    ...
    if (!process.env.RELOTO_MONGO_URI_PAYMENTS) {
        throw new Error('RELOTO_MONGO_URI_PAYMENTS must be defined');
    }
    ...
    try {
        await mongoose.connect(process.env.RELOTO_MONGO_URI_PAYMENTS, {
